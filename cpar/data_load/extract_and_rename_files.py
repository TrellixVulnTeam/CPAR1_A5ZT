...
class ExtractFiles(object):
    ...
    def create_and_unzip_files(self):
        ...
        if os.path.exists(self.output_file_path) and len([i for i in os.listdir(self.current_dir)]) == 13:
        ...
        zip_files = [i for i in os.listdir(self.current_dir) if i.endswith('CCCDMonthlyUICheck.zip')]
        ...
        if len(zip_files) != 1 :
            ...
            try:
                ...
                if os.path.exists(self.path_to_zip_file):
                    os.rename(self.path_to_zip_file, self.source_file_path+self.file_name)
                unzip_files()
            except:
